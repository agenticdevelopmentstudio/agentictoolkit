...
struct MainThreadWindowStatusBarTests {
    ...
    /// (Ruling 7), must hold even though the mutation itself always
    /// succeeds.
    @Test
    ...
    /// one call per write, with no coalescing (Divergence #1, Ruling 7):
    /// `show()` itself commits once, then two further `text` writes each
    /// commit once more, for three calls total.
    @Test

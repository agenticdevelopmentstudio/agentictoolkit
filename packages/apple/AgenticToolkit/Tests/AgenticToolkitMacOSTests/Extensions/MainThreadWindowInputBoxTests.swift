...
/// The status bar item seam, for a suite that presents no status bar item,
/// on `UnusedMessagePresenter`'s own reasoning: `MainThreadWindow.init` does
/// not default `statusBarPresenter:` either.
@MainActor
private final class UnusedStatusBarPresenter: ExtensionStatusBarPresenting {
    func putOrUpdateStatusBarItem(_ request: ExtensionStatusBarItemRequest) {
        Issue.record("putOrUpdateStatusBarItem was called by a suite that exercises no status bar item.")
    }

    func removeStatusBarItem(internalID: String) {
        Issue.record("removeStatusBarItem was called by a suite that exercises no status bar item.")
    }
}
...
struct MainThreadWindowInputBoxTests {
    ...
    private func activate(
        ...
        let window = MainThreadWindow(
            ...
            statusBarPresenter: UnusedStatusBarPresenter(),
    ...
    /// Kills a mutation that resolves a non-string value which stringifies to
    /// the presenter's answer anyway — a boxed `String`, an object with a
    /// custom `toString` — which `typeof result === 'string'` below catches
    /// and a plain string comparison (test 1's) does not.
    @Test
    ...
    /// The first call, `showInputBox({})`, is the case no earlier test
    /// reaches: an options object that is present but carries no
    /// `validateInput`. Kills a mutation that sets `isValidating = true`
    /// whenever an options object exists at all, regardless of whether
    /// `validateInput` was supplied.
    @Test
    ...
    func aMessageWithNoSeverityDefaultsToErrorAndASeverityWithNoMessageIsNil() async throws {
        ...
        let severityOnlyValidate = try #require(severityOnlyPresenter.lastValidate)
        let severityOnlyResult = await severityOnlyValidate("anything")
    ...
    // MARK: - 19. A severity no `Int32` can hold falls to `.error` instead of trapping

    /// Kills a mutation that narrows `severity` with the trapping
    /// `Int32(_:)` rather than `Int32(exactly:)`. No earlier test kills it,
    /// and not because the earlier tests are few: every severity any of them
    /// hands the validator is a small integer well inside `Int32`, so the
    /// trapping initialiser answers each without complaint and the whole file
    /// stays green while the crash ships.
    ///
    /// `1e10` and `Infinity` are the cases an integrality test lets through,
    /// which is what makes this more than a fractional-input test: both are
    /// equal to themselves rounded toward zero, so a guard written as
    /// `d == d.rounded(.towardZero)` admits them and then traps. Under the
    /// trapping initialiser this test does not fail, it crashes the test
    /// process — which is the only shape a trap regression can take, and is
    /// still the earliest signal available.
    @Test
    func aSeverityOutsideInt32FallsToTheDefaultArmRatherThanTrapping() async throws {
        let directory = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        // Each is a JavaScript number literal, spliced into the source below.
        let cases = [
            ("fractional", "2.5"),
            ("aboveInt32", "1e10"),
            ("belowInt32", "-1e10"),
        ...
        for (name, severityLiteral) in cases {
            let presenter = RecordingInputBoxPresenter()
            presenter.response = "x"
            let host = try makeHost(
                name: name,
                ...
                exports.activate = function () {
                    ...
                    vscode.window.showInputBox({
                        validateInput: function (value) {
                            return { message: 'careful', severity: \(severityLiteral) };
                        }
                    }).then(function () {
                        globalThis.__settled = { ok: true };
                    }, function (error) {
            ...
            let result = try #require(await validate("anything"), "case \(name)")
            #expect(result.message == "careful", "case \(name)")
            #expect(result.severity == .error, "case \(name)")
        }
    }
}

...
/// An `ExtensionStatusBarPresenting` double for a suite that never calls
/// `createStatusBarItem`'s `show()`, on `UnusedQuickPickPresenter`'s own
/// reasoning: `MainThreadWindow.init` does not default its
/// `statusBarPresenter:` argument either, so every construction in this file
/// has to name one, and naming one that traps says these tests exercise no
/// status bar item presentation.
///
/// `MainThreadWindowStatusBarTests` is where a status bar item is actually
/// presented, and it has its own recording double.
@MainActor

...
/// Stand-in for Task 12's real mirror projection. `resources` is empty, so
/// every sync-store call falls through to the generic JSON mirror for these
/// nine names — nothing here is exercised by `MarkdownStore`'s own tests,
/// which read its typed tables directly, not the mirror. Delete this type
/// when Task 12 lands its own `MarkdownProjection.swift`.
struct MarkdownProjection: SyncMirrorProjection {

    /// `content.markdown`, `.notes`, `.docs` and `.papers` are pull-only in
    /// `ADHSyncCatalog` — adh owns title derivation, hashing, sizing and
    /// version history for them.
    static let pullOnlyResources: [String] = ["markdown", "notes", "docs", "papers"]
        .map { "content.\($0)" }

    /// One resource per table `MarkdownSchema` creates — read from there so
    /// this list can never drift from the schema it registers against.
    static let syncResources: [SyncResource] = MarkdownSchema.tables
        .map { SyncResource(resource: "content.\($0)", schemaVersion: 1) }
    ...
    func syncVersion(resource: String, id: String, in conn: Database) throws -> Int? { nil }
    ...
    func row(resource: String, id: String, in conn: Database) throws -> [String: JSONValue]? { nil }
}

...
///
/// **`notImplementedLedger` is now read, and so is `extensionIdentifier`
/// (task 5.5c).** This paragraph used to say neither was — that was true
/// before `createStatusBarItem` existed and is not true now. `tooltip`'s and
/// `command`'s setters call `notImplementedLedger.record(memberPath:
/// extensionIdentifier:)` for a `MarkdownString` tooltip or a `Command`-object
/// command (Ruling 5 of that task's brief): values this host carries no
/// rendering or dispatch plumbing for yet. `extensionIdentifier` is read
/// twice over: it is the public `.id` an id-less status bar item answers
/// (`vscode.d.ts:7564-7570`'s own words — "if no identifier was provided [...]
/// the identifier will match the [...] extension identifier"), and it is
/// folded into the internal id an item created *with* one is keyed by, so
/// two extensions that both pass `'status'` to `createStatusBarItem` do not
/// collide in this adaptor's registry. Nothing here builds a
/// `VSCodeAPI.subNamespace` the way `MainThreadWorkspace.fs` does, so nothing
/// here has that particular kind of miss to record. `showQuickPick`'s and
/// `showInputBox`'s ignored cancellation tokens are still **not** recorded:
/// a `NotImplementedAccess` is "One VS Code API member an
...
/// thrown for a degraded cancellation token — recording
/// `vscode.window.showQuickPick` would make task 5.8's report tell a user
/// that a member which works does not exist. A degraded *argument* is not an
/// absent *member*.
///
...
public final class MainThreadWindow {
    ...
    /// Where a `createStatusBarItem` object's `show()`/property writes
    /// actually put something on screen (or, in a test, record what they
    /// were asked to show). A fourth presenter rather than a member on any
    /// existing protocol — see `ExtensionQuickPickPresenting`'s own doc for
    /// the interface-segregation reasoning this adaptor's every seam shares.
    private let statusBarPresenter: ExtensionStatusBarPresenting

    /// Where a reach for an undefined `window` member is recorded, and, as of
    /// task 5.5c, where `createStatusBarItem`'s `tooltip` and `command`
    /// setters record a value this host does not carry — see this type's own
    /// doc.
    private let notImplementedLedger: NotImplementedLedger

    /// The extension this adaptor belongs to. Read by `createStatusBarItem`
    ...
    /// crashing — into a torn-down extension. `createStatusBarItem` has no
    /// `await` to check after, so it checks this once, at the top of
    /// `handleCreateStatusBarItem()` — the "pre-await" guard on a member with
    /// no `await` at all.
    private var isDisposed = false

    /// Every live `createStatusBarItem` object, keyed by
    /// `ExtensionStatusBarItem.internalID`. The sole strong reference to each
    /// one — see `ExtensionStatusBarItem`'s own doc, Ruling 4 of task 5.5c's
    ...
    ///     of the same name. See this type's own doc for how it is used.
    public init(
        ...
        statusBarPresenter: ExtensionStatusBarPresenting,
    ...
    /// binding diverges from upstream, which invokes `validateInput` bound to
    /// its own internal instance rather than the options object
    /// (`extHostQuickOpen.ts:171`).
    ...
    /// Reads `InputBoxOptions.valueSelection` (`vscode.d.ts:2243-2249`):
    /// "Defined as tuple of two number where the first is the inclusive
    /// start index and the second the exclusive end index."
    ///
    ...
    ///    A `.threw` answers `nil`: a rejected validation promise answers
    ///    "valid" rather than surfacing, extended to a synchronous throw on
    ///    the same reasoning — smaller harm than blocking every future
    ///    keystroke on one bad call, and reversible by the extension.
    ...
    ///
    /// **`this` is bound to `optionsObject`, which diverges from upstream.**
    /// Measured against `extHostQuickOpen.ts:171`, upstream calls
    /// `this._validateInput(input)` — a plain call through the enclosing
    /// `ExtHostQuickOpen`-side class's own field, so `this` inside the
    /// extension's `validateInput` is that internal instance, never the
    /// `options` object the extension passed to `showInputBox`. This host
    /// binds `this` to `optionsObject` instead,
    ...
    ///   `3` → `.error`, and everything else → `.error`, matching the
    ///   declaration's own default for a bare string. "Everything else" is
    ///   stated that way rather than as a list of cases because the list kept
    ///   acquiring new members: absent, a non-number, and a number that is not
    ///   exactly `1`, `2` or `3` all land there, the last covering fractional,
    ///   out-of-range, infinite and NaN alike.
    ...
    private static func inputValidation(from value: JSValue) -> ExtensionInputValidation? {
        ...
        if let severityValue = value.forProperty("severity"), severityValue.isNumber {
            // `Int32(exactly:)` and not `Int32(_:)`: the trapping initialiser crashes the host on
            // any double outside `Int32`'s range, and `1e10` and `Infinity` are both integral, so
            // an integrality test does not keep them away from it. `exactly:` answers `nil` for
            // non-integral, out-of-range, infinite and NaN alike, which is the one behaviour the
            // default arm below wants from all four.
            switch Int32(exactly: severityValue.toDouble()) {
            case 1:
    ...
    // MARK: - vscode.window.createStatusBarItem

    /// This member's full path, used in its own raised-exception message and
    /// in the torn-down message `VSCodeAPI.member` builds.
    private static let createStatusBarItemMemberPath = "vscode.window.createStatusBarItem"

    /// `implementation` for `vscode.window.createStatusBarItem`, handed to
    /// `ExtensionHost.defineVSCodeMember(namespacePath:name:implementation:)`
    /// as-is. `.raisedException`, not `.rejectedPromise`: both overloads
    /// return a `StatusBarItem` synchronously (`vscode.d.ts:11641`, `:11651`)
    /// — there is no `Thenable` here to reject — matching
    /// `MainThreadWorkspace.getWorkspaceFolder`'s own reasoning for the same
    /// choice on a synchronous member.
    public private(set) lazy var createStatusBarItem: Any = VSCodeAPI.member(
        MainThreadWindow.createStatusBarItemMemberPath, of: self, whenTornDown: .raisedException
    ) { $0.handleCreateStatusBarItem() }

    /// The `vscode.StatusBarAlignment` enum (`vscode.d.ts:7542-7556`: `Left =
    /// 1`, `Right = 2`), as the name→value table an installer hands to
    /// `ExtensionHost.defineVSCodeMember(namespacePath:name:implementation:)`
    /// — `quickPickItemKindMembers`'s own pattern and own reason: an
    /// extension that writes `vscode.StatusBarAlignment.Right` needs that
    /// expression to resolve.
    ///
    /// **`"vscode"`, not `"vscode.window"`, is the namespace path an
    /// installer must use.** `StatusBarAlignment` is a top-level `vscode`
    ...
    /// answer the shape it was given — a plain string back for a plain
    /// string write, a freshly-minted `{id: ...}` object for a `ThemeColor`
    /// write — rather than always answering one shape. Holds only a `String`
    ...
    private enum ExtensionStatusBarColorValue: Equatable {
        ...
        /// The flat id this value carries either way — what
        /// `ExtensionStatusBarItemRequest.color`/`.backgroundColor`
        /// (`String?`) actually stores.
        var id: String {
            switch self {
            case .string(let value): return value
            case .themeColor(let id): return id
            }
        }
    }
    ...
    private final class ExtensionStatusBarItem {
        ...
        /// The registry key. Never shown to the extension — see
        /// `ExtensionStatusBarItemRequest.internalID`'s own doc.
        let internalID: String

        /// `StatusBarItem.id` (`vscode.d.ts:7564-7570`), fixed at creation:
        /// the declaration gives `id` no setter, and neither does this class.
        let id: String

        /// `StatusBarItem.alignment` (`:7572-7575`), fixed at creation for
        /// `id`'s own reason.
        let alignment: ExtensionStatusBarAlignment

        /// `StatusBarItem.priority` (`:7577-7581`), fixed at creation for
        /// `id`'s own reason. `nil` means "undefined", never `0` — Ruling 8.
        let priority: Double?

        var name: String?
        var text = ""
        var tooltip: String?
        var color: ExtensionStatusBarColorValue?
        var backgroundColor: ExtensionStatusBarColorValue?
        var command: String?
        var accessibilityLabel: String?
        var accessibilityRole: String?

        /// Set by `MainThreadWindow.showStatusBarItem`, cleared by
        /// `hideStatusBarItem`. Upstream's `update()` returns immediately
        /// when `!_visible` (`extHostStatusBar.ts:245-247`); this flag is
        /// that same early return, read by `commitStatusBarUpdate`.
        var isVisible = false
        ...
        init(internalID: String, id: String, alignment: ExtensionStatusBarAlignment, priority: Double?) {
            self.internalID = internalID
            self.id = id
            self.alignment = alignment
            self.priority = priority
        ...
        /// The presenter-facing snapshot of this item's current state —
        /// what `commitStatusBarUpdate` hands `statusBarPresenter
        /// .putOrUpdateStatusBarItem(_:)`.
        var request: ExtensionStatusBarItemRequest {
            ExtensionStatusBarItemRequest(
                internalID: internalID, id: id, alignment: alignment, priority: priority,
                name: name, text: text, tooltip: tooltip, color: color?.id, backgroundColor: backgroundColor?.id,
                command: command, accessibilityLabel: accessibilityLabel, accessibilityRole: accessibilityRole)
        }
    }

    /// Rejects — raises, rather; see `createStatusBarItem`'s own doc — on a
    /// torn-down adaptor. **This member has no `await` to re-check
    ...
    /// (`VSCodeAPI.swift`'s `tornDown(path:response:)`) verbatim, so a caller
    /// sees one sentence regardless of which of the two produced it.
    ///
    /// Argument shape, from `vscode.d.ts:11633-11641`, `:11643-11651`:
    /// `createStatusBarItem(id: string, alignment?: StatusBarAlignment,
    /// priority?: number): StatusBarItem` and `createStatusBarItem(alignment?:
    /// StatusBarAlignment, priority?: number): StatusBarItem`.
    /// "JavaScript has no overloads, so this is one function that
    /// discriminates on its first argument's type" — task 5.5c's brief,
    /// stating the rule this method follows: **argument 0's `isString` is
    /// the entire discriminator.** A string argument 0 is the `id` form
    /// (`id` = that string; `alignment` shifts to argument 1; `priority` to
    /// argument 2). Anything else — nothing at all, a number, a boolean, an
    /// object — is the alignment form (`alignment` = argument 0; `priority`
    /// = argument 1). **A caller passing something that is neither a string
    /// nor a number as argument 0 is not rejected**: it silently falls into
    /// the alignment form, and `parseAlignment(_:)` then answers `.left` for
    /// it — the same answer an absent argument gets — because neither
    /// overload above declares a third shape to reject into.
    private func handleCreateStatusBarItem() -> JSValue? {
        guard let context = JSContext.current() else { return nil }
        let path = MainThreadWindow.createStatusBarItemMemberPath
        ...
        let id: String?
        let alignmentArgument: JSValue?
        let priorityArgument: JSValue?
        if let firstArgument, firstArgument.isString {
            id = firstArgument.toString()
            alignmentArgument = arguments.count > 1 ? arguments[1] : nil
            priorityArgument = arguments.count > 2 ? arguments[2] : nil
        } else {
            id = nil
            alignmentArgument = firstArgument
            priorityArgument = arguments.count > 1 ? arguments[1] : nil
        }
        ...
        let item = ExtensionStatusBarItem(
            internalID: internalID, id: publicID, alignment: alignment, priority: priority)
        statusBarItems[internalID] = item
        return MainThreadWindow.makeStatusBarItemObject(for: item, of: self, in: context)
    }

    /// `StatusBarAlignment` (`vscode.d.ts:7542-7556`): raw JS `2` maps to
    /// `.right`; every other value — absent, a non-number, `1`, or any other
    /// number — maps to `.left`, matching upstream's own default parameter
    /// (`extHostStatusBar.ts:63`, which defaults `alignment` to
    /// `ExtHostStatusBarAlignment.Left`). The raw numbers `1`/`2` map onto
    /// `ExtensionStatusBarAlignment`'s cases only here and in
    /// `statusBarAlignmentMembers` — see that enum's own doc.
    private static func parseAlignment(_ value: JSValue?) -> ExtensionStatusBarAlignment {
        guard let value, value.isNumber, value.toInt32() == 2 else { return .left }
        return .right
    }

    /// `StatusBarItem.priority` (`vscode.d.ts:7577-7581`): `number |
    /// undefined`. `nil` for absent, `undefined`, `null`, or any non-number
    /// — **never coerced to `0`** (Ruling 8). A JS number is taken verbatim
    /// via `toDouble()`.
    ///
    /// **Divergence #2, no infinity clamping:** upstream's `validatePriority`
    /// (`extHostStatusBar.ts:92-111`) replaces `±Infinity` with
    /// `±Number.MAX_VALUE` because its RPC layer serialises through JSON,
    /// which cannot carry `Infinity` (`:97-99`, citing
    /// `microsoft/vscode#133317`). `MainThreadWindow` and
    /// `ExtensionStatusBarPresenting` share one process and call each other
    /// directly — there is no JSON boundary here for a clamp to protect —
    /// so `±Infinity` passes through unchanged.
    private static func parsePriority(_ value: JSValue?) -> Double? {
        guard let value, value.isNumber else { return nil }
        return value.toDouble()
    }

    /// Reads a `color`/`backgroundColor` write: `string | ThemeColor |
    /// undefined` for `color` (`vscode.d.ts:7605-7608`), `ThemeColor |
    /// undefined` for `backgroundColor` (`:7610-7622`) — one reader serves
    /// both, since a `ThemeColor`'s shape does not depend on which property
    ...
    /// have a shape this host recognizes but cannot yet act on
    /// (`MarkdownString`, `Command`) — `color`/`backgroundColor` do not: any
    /// shape besides a string or an `{id}`-bearing object is simply not a
    /// colour.
    ///
    /// **This task's `backgroundColor` carries whatever `ThemeColor` id
    /// arrives, unfiltered — Divergence #4.** Upstream's setter silently
    /// discards any id outside a two-entry allow-list
    /// (`extHostStatusBar.ts:194-201`, against the map declared at `:23-28`)
    ...
    private static func colorValue(from value: JSValue?) -> ExtensionStatusBarColorValue? {
        ...
        guard value.isObject, let idValue = value.forProperty("id"), idValue.isString,
              let id = idValue.toString() else {
            return nil
        }
        return .themeColor(id: id)
    }

    /// The `color`/`backgroundColor` getter's answer: the original string
    /// back for a `.string`, or — for a `.themeColor` — a freshly minted
    /// plain object with one `id` property.
    ///
    /// **Divergence #3: the minted object is not the one the extension set,
    /// and it carries no `ThemeColor` prototype** — this host defines no
    ...
    /// Installs a `defineProperty` getter with no setter — for `id`,
    /// `alignment` and `priority`, which `vscode.d.ts:7570`, `:7575`,
    /// `:7581` all declare `readonly`. Assigning to one of these from
    /// JavaScript is then a silent no-op — non-strict-mode JavaScript's own
    /// behaviour for writing a property whose descriptor has no setter, not
    ...
    /// Builds the JS-visible `StatusBarItem` object for `item`:
    ...
    /// `JSContext` can reach contains nothing this factory minted. A block
    ...
    private static func makeStatusBarItemObject(
        ...
        installReadonlyGetter(on: object, name: "id") { [weak item] in item?.id }
        installReadonlyGetter(on: object, name: "alignment") { [weak item] in
            guard let item else { return nil }
            return item.alignment == .right ? 2 : 1
        }
        ...
        installAccessor(on: object, name: "text",
            get: { [weak item] in item?.text ?? "" },
            set: { [weak item, weak window] value in
                guard let item, let window, let value, value.isString, let string = value.toString() else {
                    return
                }
                item.text = string
                window.commitStatusBarUpdate(item)
        ...
        installAccessor(on: object, name: "color",
            get: { [weak item] in
                guard let color = item?.color, let context = JSContext.current() else { return nil }
                return MainThreadWindow.makeColorObject(for: color, in: context)
            },
            set: { [weak item, weak window] value in
                guard let item, let window else { return }
                item.color = MainThreadWindow.colorValue(from: value)
                window.commitStatusBarUpdate(item)
            })

        installAccessor(on: object, name: "backgroundColor",
            get: { [weak item] in
                guard let color = item?.backgroundColor, let context = JSContext.current() else { return nil }
                return MainThreadWindow.makeColorObject(for: color, in: context)
            },
            set: { [weak item, weak window] value in
                guard let item, let window else { return }
                item.backgroundColor = MainThreadWindow.colorValue(from: value)
                window.commitStatusBarUpdate(item)
            })

        installAccessor(on: object, name: "accessibilityInformation",
            get: { [weak item] in
                guard let item, let label = item.accessibilityLabel, let context = JSContext.current(),
                      let infoObject = JSValue(newObjectIn: context) else {
                    return nil
                }
                infoObject.setObject(label, forKeyedSubscript: "label" as NSString)
                if let role = item.accessibilityRole {
                    infoObject.setObject(role, forKeyedSubscript: "role" as NSString)
                }
                return infoObject
            },
            set: { [weak item, weak window] value in
                ...
                guard let value, value.isObject,
                      let labelValue = value.forProperty("label"), labelValue.isString,
                      let label = labelValue.toString() else {
                    return
                }
        ...
        let show: @convention(block) () -> Void = { [weak item, weak window] in
            MainActor.assumeIsolated {
                guard let item, let window else { return }
                window.showStatusBarItem(item)
            }
        }
        object.setObject(show, forKeyedSubscript: "show" as NSString)

        let hide: @convention(block) () -> Void = { [weak item, weak window] in
            MainActor.assumeIsolated {
                guard let item, let window else { return }
                window.hideStatusBarItem(item)
            }
        }
        object.setObject(hide, forKeyedSubscript: "hide" as NSString)

        var disposed = false
        let dispose: @convention(block) () -> Void = { [weak item, weak window] in
            MainActor.assumeIsolated {
                guard !disposed else { return }
                disposed = true
                guard let item, let window else { return }
                window.disposeStatusBarItem(item)
            }
        }
    ...
    /// `StatusBarItem.show()` (`vscode.d.ts:7639-7642`). Marks `item`
    /// visible and commits — the first commit an item ever makes, since
    /// nothing before `show()` calls `commitStatusBarUpdate`.
    ///
    /// **A disposed item ignores this call.** There is no test-visible
    /// difference between that and letting it fall through to
    /// `commitStatusBarUpdate`'s own `!isDisposed` guard below, but stating
    /// it here rather than only there means a reader never has to chase two
    /// guards to answer "does `show()` do anything after `dispose()`".
    private func showStatusBarItem(_ item: ExtensionStatusBarItem) {
        guard !item.isDisposed else { return }
        item.isVisible = true
        commitStatusBarUpdate(item)
    }

    /// Pushes `item`'s current state to `statusBarPresenter`, or does
    /// nothing — mirroring upstream's own early return in `update()`:
    /// "if (this._disposed) { return; }" then "if (!this._visible) {
    /// return; }" (`extHostStatusBar.ts:245-247`). **No debounce —
    /// Divergence #1.** Upstream defers every update through `setTimeout(…,
    /// 0)`, stated reason "Defer the update so that multiple changes to
    /// setters dont cause a redraw each" (`:251`) — a cross-process JSON-RPC
    ...
    /// `StatusBarItem.hide()` (`vscode.d.ts:7644-7647`).
    ///
    /// **Removes unconditionally, not only when previously visible** —
    /// mirroring upstream's own `hide()`, which always calls
    /// `$disposeEntry` regardless of `_visible`'s prior value
    /// (`extHostStatusBar.ts:238-242`: "There is no `$hideEntry`"). Calling
    /// `hide()` on an item that was never shown is therefore not a no-op on
    /// the presenter side — it removes an id the presenter never put up,
    /// which is why `ExtensionStatusBarPresenting.removeStatusBarItem(
    /// internalID:)`'s own doc says a conformer must tolerate that.
    private func hideStatusBarItem(_ item: ExtensionStatusBarItem) {
        guard !item.isDisposed else { return }
        item.isVisible = false
        statusBarPresenter.removeStatusBarItem(internalID: item.internalID)
    }

    /// `StatusBarItem.dispose()` (`vscode.d.ts:7649-7653`, "Dispose and free
    /// associated resources. Call {@link StatusBarItem.hide hide}."). Idempotent
    /// (test 16): the `!item.isDisposed` guard makes a second call a no-op
    /// before it touches anything.
    ///
    /// **Order mirrors upstream's own `dispose()`** — `this.hide();` first,
    /// `this._disposed = true;` after (`extHostStatusBar.ts:303-309`) — so
    /// `hideStatusBarItem` runs while `isDisposed` is still `false` and
    /// actually reaches the presenter, rather than being skipped by a flag
    ...
    private func disposeStatusBarItem(_ item: ExtensionStatusBarItem) {
        ...
        statusBarItems.removeValue(forKey: item.internalID)
    }

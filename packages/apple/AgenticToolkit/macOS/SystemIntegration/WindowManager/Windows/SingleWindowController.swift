...
open class SingleWindowController: NSWindowController, NSWindowDelegate {
    ...
    public func fitWindow(toContentSize contentSize: NSSize) {
        ...
        let fit = FrameCalculator.contentHuggingFrame(
            currentFrame: window.frame,
            desiredFrameSize: frameSize,
            screenVisibleFrame: screen.visibleFrame,
            minSize: windowSpec?.minSize ?? window.minSize,
            anchors: anchorsToHold(on: screen)
        )
        let target = fit.frame
        ...
        // (a fit that moves the window can move it onto another one). The
        // anchors go with it, for the same reason and at the same moment.
        defer {
            heldAnchors = fit.anchors
    ...
    /// The screen edges the running gesture is anchored to, as of the last fit.
    ///
    /// A content-hugging resize is reversible under a start anchor and
    /// reversible under an end anchor, but under a rule that re-reads the
    /// anchor from the frame at each step it is reversible under neither — a
    /// text-size slider dragged out and back walks the window across the
    /// screen. `FrameCalculator.contentHuggingFrame` spells out why. So the
    /// anchor is chosen on the first step and held for the rest.
    private var heldAnchors: FrameAnchors?

    /// The anchors to hold, or `nil` to let the fit choose fresh ones.
    ///
    /// Held only while the window is exactly where our own last fit left it, on
    /// the same visible area. Anything else — the user dragging the window,
    /// another window manager moving it, the Dock changing the visible area, a
    /// move to a second display — ends the gesture: the anchor was a reading of
    /// *that* position, and the new position deserves its own.
    private func anchorsToHold(on screen: any ScreenInfo) -> FrameAnchors? {
        guard let heldAnchors,
              let window,
              let last = lastContentFit,
              last.windowFrame == window.frame,
              last.screenVisibleFrame == screen.visibleFrame
        else { return nil }
        return heldAnchors
    }

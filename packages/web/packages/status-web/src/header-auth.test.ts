...
// The auth/me semantics that keep a logged-in user logged in across backend
...
describe("fetchStatusUser", () => {
  ...
  it("propagates a network failure so the cached session is kept", async () => {
    ...
    await expect(fetchStatusUser(api, failing)).rejects.toThrow(/fetch failed/);
  });
});

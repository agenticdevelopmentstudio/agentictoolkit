...
// caller's `StatusApiClient` proxy: `/config/*` → `BACKEND_URL/config/*`
...
// PATCH, not PUT). This module never names a base path — every function takes
// the caller's `StatusApiClient` explicitly.
...
const GROUPS = "/config/site-groups";
...
// --- groups -----------------------------------------------------------------
export async function listGroups(api: StatusApiClient): Promise<SiteGroupView[]> {
  return (await api.json<GroupRow[]>(GROUPS)).map(toGroup);
}
export async function createGroup(api: StatusApiClient, body: { name: string; slug: string; retentionDays?: number }): Promise<SiteGroupView> {
  const fields: { name: string; slug: string; retentionDays?: number } = {
    name: body.name,
    slug: body.slug,
    retentionDays: body.retentionDays,
  };
  return toGroup(await api.json<GroupRow>(GROUPS, { method: "POST", body: JSON.stringify(fields) }));
}
export async function updateGroup(api: StatusApiClient, id: string, body: { name?: string; slug?: string; retentionDays?: number }): Promise<SiteGroupView> {
  const fields: { name?: string; slug?: string; retentionDays?: number } = {
    name: body.name,
    slug: body.slug,
    retentionDays: body.retentionDays,
  };
  return toGroup(await api.json<GroupRow>(`${GROUPS}/${id}`, { method: "PATCH", body: JSON.stringify(fields) }));
}
export function deleteGroup(api: StatusApiClient, id: string): Promise<void> {
  return api.json<void>(`${GROUPS}/${id}`, { method: "DELETE" });
}

// --- sites ------------------------------------------------------------------
export async function listSites(api: StatusApiClient): Promise<SiteView[]> {
  return (await api.json<SiteRow[]>(SITES)).map(toSite);
}
export async function createSite(api: StatusApiClient, body: { name: string; slug: string; groupId: string }): Promise<SiteView> {
  const fields: { name: string; slug: string; siteGroupId: string } = {
    name: body.name,
    slug: body.slug,
    siteGroupId: body.groupId,
  };
  return toSite(await api.json<SiteRow>(SITES, { method: "POST", body: JSON.stringify(fields) }));
}
export async function updateSite(api: StatusApiClient, id: string, body: { name?: string; slug?: string; groupId?: string }): Promise<SiteView> {
  const fields: { name?: string; slug?: string; siteGroupId?: string } = {
    name: body.name,
    slug: body.slug,
    siteGroupId: body.groupId,
  };
  return toSite(await api.json<SiteRow>(`${SITES}/${id}`, { method: "PATCH", body: JSON.stringify(fields) }));
}
...
export function deleteSite(api: StatusApiClient, id: string): Promise<void> {
  return api.json<void>(`${SITES}/${id}`, { method: "DELETE" });
}

// --- endpoints --------------------------------------------------------------
export async function listEndpoints(api: StatusApiClient, siteId: string): Promise<EndpointView[]> {
  return (await api.json<EndpointRow[]>(`${ENDPOINTS}?siteId=${encodeURIComponent(siteId)}`)).map(toEndpoint);
}
/** Every endpoint across all sites (the route returns all when no siteId is given). */
export async function listAllEndpoints(api: StatusApiClient): Promise<EndpointView[]> {
  return (await api.json<EndpointRow[]>(ENDPOINTS)).map(toEndpoint);
}
...
export async function createEndpoint(api: StatusApiClient, siteId: string, body: Partial<Omit<EndpointView, "id" | "siteId">> & { url: string }): Promise<EndpointView> {
  ...
  return toEndpoint(await api.json<EndpointRow>(ENDPOINTS, { method: "POST", body: JSON.stringify(fields) }));
}
export async function updateEndpoint(api: StatusApiClient, id: string, body: Partial<Omit<EndpointView, "id" | "siteId">>): Promise<EndpointView> {
  ...
  return toEndpoint(await api.json<EndpointRow>(`${ENDPOINTS}/${id}`, { method: "PATCH", body: JSON.stringify(fields) }));
}
export function deleteEndpoint(api: StatusApiClient, id: string): Promise<void> {
  return api.json<void>(`${ENDPOINTS}/${id}`, { method: "DELETE" });
}
...
export function listIntegrations(api: StatusApiClient): Promise<IntegrationView[]> {
  return api.json<IntegrationView[]>(INTEGRATIONS);
}
export function createIntegration(api: StatusApiClient, body: { platform: string; label: string; config?: Record<string, unknown>; tokenEnvVar?: string | null; isActive?: boolean }): Promise<IntegrationView> {
  return api.json<IntegrationView>(INTEGRATIONS, { method: "POST", body: JSON.stringify(body) });
}
export function updateIntegration(api: StatusApiClient, id: string, body: { platform?: string; label?: string; config?: Record<string, unknown>; tokenEnvVar?: string | null; isActive?: boolean }): Promise<IntegrationView> {
  return api.json<IntegrationView>(`${INTEGRATIONS}/${id}`, { method: "PATCH", body: JSON.stringify(body) });
}
export function deleteIntegration(api: StatusApiClient, id: string): Promise<void> {
  return api.json<void>(`${INTEGRATIONS}/${id}`, { method: "DELETE" });
}

// --- ignored deploy projects ------------------------------------------------
const IGNORED = "/config/ignored-projects";

export interface IgnoredProjectView {
  platform: string;
  projectName: string;
}
export function listIgnoredProjects(api: StatusApiClient): Promise<IgnoredProjectView[]> {
  return api.json<IgnoredProjectView[]>(IGNORED);
}
export function ignoreProject(api: StatusApiClient, platform: string, projectName: string): Promise<void> {
  return api.json<void>(IGNORED, { method: "POST", body: JSON.stringify({ platform, projectName }) });
}
/**
 ...
 */
export async function ignoreProjects(api: StatusApiClient, projects: { platform: string; projectName: string }[]): Promise<void> {
  await Promise.all(projects.map((p) => ignoreProject(api, p.platform, p.projectName)));
}
export function unignoreProject(api: StatusApiClient, platform: string, projectName: string): Promise<void> {
  // Backend DELETE keys off a compound id: `${platform}|${projectName}` (URL-encoded).
  const id = encodeURIComponent(`${platform}|${projectName}`);
  return api.json<void>(`${IGNORED}/${id}`, { method: "DELETE" });
}

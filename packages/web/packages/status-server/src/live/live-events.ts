 ...
 *
 * The producer (a finished cycle, or a webhook) calls `emitLiveUpdate(storage, config)`; we
...
export function emitLiveUpdate(storage: Storage, config: StatusConfig): void {
  ...
  pending = setTimeout(() => {
    pending = null;
    buildLiveSnapshot(storage, config)

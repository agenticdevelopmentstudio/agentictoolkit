...
import type { PlatformObservationInput } from "../src/storage/ports";
...
// `storage.observations.recordObservations` is now the SINGLE writer of `consecutive_failures`:
// the second writer (`applyPlatformIssues`) is gone, and with it the double-advance this file
...
async function problemsFrom(db: Awaited<ReturnType<typeof freshDb>>) {
  ...
  return deriveBoard(await readBoardFacts(createLibsqlStorage(db), nowMs, testConfig()), nowMs).problems;
}
...
describe("recordObservations", () => {
  it("records what the poll saw AND advances the streak — one call, one writer", async () => {
    ...
    const obs: PlatformObservationInput[] = [{ source: "vercel", configured: true, reachable: false }];
    await storage.observations.recordObservations(obs);
  ...
  it("records an UNCONFIGURED platform without ever accruing a streak", async () => {
    const db = await freshDb();
    const storage = createLibsqlStorage(db);
    await storage.observations.recordObservations([{ source: "crunchy", configured: false, reachable: false }]);
    const [row] = await db.select().from(platformHealthState);

...
its name" (line 7) once some other component re-points the row. `ScannedGitRepo`
...
because a scan result "has no identity yet" (lines 53-55). `ProjectScanSummary`
...
- **git-repo-stored-shape**: `GitRepo` MUST store exactly seven properties —
  ...
  conform to `Sendable`, `Identifiable`, and `Equatable` (lines 9-21).
- **git-repo-id-immutable**: `GitRepo.id` MUST be a `let` constant set only at
  ...
  `init` returns (line 11).
- **git-repo-default-id-generation**: `init` MUST generate a fresh, distinct
  `UUID` for `id` via `UUID()` when the caller supplies none (line 24).
- **git-repo-mutable-tracking-fields**: `path`, `name`, `remote`, `firstSeen`,
  ...
  constructing a new value (lines 13-21).
...
- **git-repo-default-name-from-path**: `GitRepo.defaultName(forPath:)` MUST
  ...
  confirm that path exists or is a directory (lines 47-50).
- **git-repo-name-seeding-convention**: Per the doc comment on `name` ("Seeded
  from the directory name, then the user's to change," line 14), a caller
...
- **git-repo-structural-equatable**: `GitRepo`'s `Equatable` conformance MUST
  ...
  timestamp, MUST compare unequal (line 9, synthesized memberwise
  conformance over lines 11-21).
- **git-repo-path-absoluteness**: The doc comment on `path` (line 12) declares it "Absolute path to the working tree," a caller precondition: neither `init` (lines 23-39) nor `url`/`defaultName(forPath:)` (lines 41-50) validates that a supplied string is non-empty or absolute, so a relative path, an empty string, or a path containing `~` is stored and used unchanged.
...
- **scanned-git-repo-remote-required**: `ScannedGitRepo.init` MUST NOT
  ...
  `GitRepo.init`, which defaults `remote` to `nil` (line 60, contrast line
  27).
...
- **scan-summary-default-zero-counts**: `ProjectScanSummary.init()` MUST take
  ...
  `skipped` at their declared default of `0` (lines 69-80).
- **scan-summary-fields-unvalidated**: `added`, `moved`, `removed`,
  ...
  others, and no property on this type MUST detect or reject that (lines
  70-78).
- **scan-summary-found-excludes-removed**: `ProjectScanSummary.found` MUST
  ...
  "every project in the registry once the scan has been applied" (lines
  82-83).
- **scan-summary-text-headline-pluralization**: `summaryText`'s headline MUST
  ...
  every other value of `found`, including `0` (line 92).
- **scan-summary-text-part-selection**: `summaryText` MUST append `"<n> new"`,
  ...
  parts list under any circumstance, regardless of its value (lines 87-91).
- **scan-summary-text-joining**: When at least one part is selected,
  ...
  `", no changes"` to the headline (lines 93-94).
- **sendable-value-semantics**: `GitRepo`, `ScannedGitRepo`, and
  ...
  conformance (lines 9, 56, 69) and their composition entirely from
...
| git-client-projects-git-repo-003 | scanned-git-repo-leaf-name | `GitRepoScannerTests.swift` line 30: scan a directory named `alpha` containing a valid `.git`, then read `found.first?.leafName`. | Returns `"alpha"`. |
| git-client-projects-git-repo-004 | scan-summary-text-headline-pluralization, scan-summary-text-joining | `ProjectReconcilerTests.swift` line 191 (`testAQuietScanSaysSo`): a plan whose summary has `unchanged: 1` and every other count `0`. | `plan.summary.summaryText == "1 project, no changes"`. |
| git-client-projects-git-repo-005 | scan-summary-text-part-selection, scan-summary-text-joining | `ProjectReconcilerTests.swift` line 182 (`testTheSummaryCountsEveryRepoExactlyOnce`): a plan whose summary has `added: 1`, `moved: 1`, `removed: 1`, `unchanged: 1`. | `plan.summary.summaryText == "3 projects — 1 new, 1 moved, 1 removed"` (the one `unchanged` project is counted in `found` but never named). |
...
- **Null and empty input**: `path = ""` is accepted unchanged by every
  initializer that takes it (lines 25, 33, 60-62) and is forwarded verbatim
  into `url`, `defaultName(forPath:)`, and `leafName` (lines 45, 48-50, 65),
  ...
  neither type normalizes the two together (MUST, lines 17, 27, 58, 60-62).
...
- **Concurrent access**: All three types are `Sendable` value types with no
  shared mutable reference state, no lock, and no actor isolation (lines 9,
  56, 69); every read or write operates on the caller's own copy, so
...
- **Missing file or unreachable directory**: A `path` naming a directory
  ...
  `FileManager` to check existence (MUST, lines 45, 48-50, 65).
...
`ProjectScanSummary.summaryText` (lines 86-95) builds a hardcoded English
...
|-----------|-------------|---------|
| none — literal in source | `"<n> project"` / `"<n> projects"` | Headline noun, singular when `found == 1` (line 92). |
| none — literal in source | `", no changes"` | Appended to the headline when no part is selected (line 93). |
| none — literal in source | `" — "` | Separator between the headline and the joined parts (line 94). |
| none — literal in source | `"<n> new"` | Appended when `added > 0` (line 88). |
| none — literal in source | `"<n> moved"` | Appended when `moved > 0` (line 89). |
| none — literal in source | `"<n> removed"` | Appended when `removed > 0` (line 90). |
| none — literal in source | `"<n> not scanned"` | Appended when `skipped > 0` (line 91) — note the field is named `skipped` but the displayed word is "not scanned". |
| none — literal in source | `", "` | Joiner between multiple selected parts (line 94). |
...
— the same reason `GitRepo.init` (lines 23-31) and `ScannedGitRepo.init`
(lines 60-63) are both written out explicitly rather than relied upon as
synthesized.
**Approved**: pending
...
registry once the scan has been applied" (line 82) — a repository the scan
...
already known is the reconciler's job" (lines 53-55) — giving `ScannedGitRepo`
...
`GitRepoScannerTests.swift` exercises `ScannedGitRepo.leafName` incidentally
(line 30) and `ProjectReconcilerTests.swift` exercises
`ProjectScanSummary.summaryText` through two scenarios (lines 182, 191), but

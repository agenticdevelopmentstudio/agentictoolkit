...
title: SecretStoring
domain: agentictoolkit://recipes/ai-plugin-runtime-ai-plugin-kit-secret-storing
type: ingredient
version: 1.0.0
status: review
language: en
...
| secret-storing-005 | set-delete-discardable-result | `store.set("x", forKey: key)` called as a bare statement with the result unused | Compiles with no "result unused" warning, because `set` is `@discardableResult` — `SecretStoring.swift:911` |
...
| secret-storing-008 | keychain-secret-store-delegation | Read the bodies of `KeychainSecretStore.get`, `.set`, `.delete` | Each is a single-expression call to the corresponding `KeychainHelper` static function with no other statement — `SecretStoring.swift:934-940` |
| secret-storing-009 | keychain-secret-store-stateless, keychain-secret-store-init | Construct `KeychainSecretStore()` twice and call `.get(forKey:)` on the same key from each instance | Both instances return the identical result, since neither holds instance state — `SecretStoring.swift:927-932` |
| secret-storing-010 | legacy-service-constant | Read `KeychainSecretStore.legacyService` | Equals `"com.agentictoolkit"`, matching `KeychainHelper.service`'s own default-fallback literal — `SecretStoring.swift:930`, `KeychainHelper.swift:10` |

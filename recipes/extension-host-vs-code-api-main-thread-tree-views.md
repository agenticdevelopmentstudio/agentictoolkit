...
- **children-treats-undefined-null-as-no-children**: `children(of:)` MUST treat a `getChildren` return value (or its settled thenable value) of JavaScript `undefined` or `null` as "no children," returning `[]` without logging, per `ProviderResult<T[]>`'s own `T[] | undefined | null | Thenable<...>` shape (`vscode.d.ts:11361`).

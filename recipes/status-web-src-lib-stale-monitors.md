...
domain: agentictoolkit://recipes/status-web-src-lib-stale-monitors
type: ingredient
version: 1.0.0
status: review
language: en
...
tags: []
depends-on:
- agentictoolkit://recipes/status-web-src-lib-live-types
related:
...
The input rows are `LiveServiceDTO` values from the live snapshot ([Live Types](agentictoolkit://recipes/status-web-src-lib-live-types)). The sole production call site is the `StaleMonitorsBanner` component, which calls the function with a day-floored clock and turns each row into a retire button backed by `deleteEndpoint(slug)`.

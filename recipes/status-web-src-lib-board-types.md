...
domain: agentictoolkit://recipes/status-web-src-lib-board-types
type: ingredient
version: 1.0.0
status: review
language: en
...
- agentictoolkit://recipes/status-web-hooks-use-activity-history
...
- **Malformed payload**: A response that does not match `Board` is not detected by this module; behavior is owned by the consumer that parses it (see [useBoard](agentictoolkit://recipes/status-web-hooks-use-board)). MUST (fact of the contract).

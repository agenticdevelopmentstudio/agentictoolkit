...
title: useTelemetry
domain: agentictoolkit://recipes/status-web-hooks-use-telemetry
type: ingredient
version: 1.0.0
status: review
language: en
...
- **live-body-cast**: A 2xx response body MUST be parsed as JSON and returned as a `TelemetrySnapshot` by cast, with no shape validation; a body that is not JSON rejects the query with the JSON parse error. The response shape is owned by the backend `/telemetry` route (see [Status Server Telemetry](agentictoolkit://recipes/status-server-telemetry)).

...
domain: agentictoolkit://recipes/status-web-src-lib-filter
type: ingredient
version: 1.0.0
status: review
language: en
...
Its one call site is `ActivityPanel` (`src/components/ActivityPanel.tsx`). Inside a `useMemo`, the panel keeps a row when it passes the source filter ([useSourceFilter](agentictoolkit://recipes/status-web-hooks-use-source-filter)) AND `matchesQuery(rowSearchText(r), q)` is true. `q` is the raw text of the panel's search box. `rowSearchText` (`src/lib/row-model.ts`) joins the row's `name`, `environment`, `statusWord`, `detail` and `commitBody` with single spaces. The module holds no state and does no I/O.

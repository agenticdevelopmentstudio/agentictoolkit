...
though its content is unreadable underneath. The scrim is a real, opaque
subview rather than an alpha applied to the content, so it can also swallow
pointer clicks and keystrokes meant for that content, while still letting an
...
- **invokes-add-callback-on-tap**: Tapping the Add button MUST invoke the
  `onAdd` closure when one is set, and MUST have no observable effect when
  `onAdd` is `nil`.
- **invokes-remove-callback-on-tap**: Tapping the Remove button MUST invoke
  the `onRemove` closure when one is set, and MUST have no observable effect
  when `onRemove` is `nil`.
- **invokes-done-callback-on-tap**: Tapping the Done button MUST invoke the
  `onDone` closure when one is set, and MUST have no observable effect when
  `onDone` is `nil`.
...
- **move-menu-rebuilt-on-refresh**: `refreshAvailability()` MUST discard and
  ...
  item per `Direction`, in `Direction.allCases` order (Left, Right, Up,
  Down).
...
- **move-items-carry-prefixed-identifiers**: Each Move menu item's
  accessibility identifier MUST be the caller-supplied prefix followed by a
  period and that item's lowercased movement name (e.g.
  `composable-tabs.arrange.move.left`).
...
- **scrim-blocks-clicks-to-content**: Component MUST cover the pane's content
  so pointer clicks and keystrokes aimed at that content do not reach it
  while the overlay is present.
...
| composable-tabs-arrange-overlay-029 | move-items-carry-prefixed-identifiers | Call `refreshAvailability()` with `accessibilityPrefix: "composable-tabs.arrange.move"` (the prefix `buildToolbar()` uses) | The "Left" item's identifier is `composable-tabs.arrange.move.left` |
...
- Boundary values (MUST): `availableDirections` can return the empty set
  ...
  state on the next `refreshAvailability()` call. Per the source's own
  comments, the caller is expected to return `false` from `canRemove` for
  the last remaining pane and to omit a direction from `availableDirections`
  when the pane is already at that edge of the window (e.g. no `Up` for a
  pane already at the top) — this view has no independent knowledge of
  "last pane" or "top of window" itself, it only renders whatever the
  closures currently report.
...
- **SwiftUI**: Layer a `ZStack` of the pane's content behind a
  `Color(role: .windowBackground).opacity(0.72)` scrim, and center a
  `VStack(spacing: 10)` of the pane-name `Text` above an `HStack(spacing: 8)`
  toolbar (`RoundedRectangle(cornerRadius: 8)` background in
...
- **Compose**: Use a `Box` whose bottom layer is the pane's content and
  ...
  `Row` of `IconButton`s, 8dp corner radius, 1dp border, elevatedSurface
  background) via `Modifier.align(Alignment.Center)`. Give the scrim's `Box`
  a `pointerInput` block that consumes events (mirroring
...
- **React/Web**: An absolutely positioned `<div>` covering the pane's
  ...
  auto` already gives `scrim-blocks-clicks-to-content` for free; give the
  scrim's own `onClick` handler (one that does not call
  `stopPropagation()`) a call to the same "select this pane" handler the
  content container uses, mirroring `scrim-click-still-reaches-pane-selection`
  via the DOM's own event-bubbling to an ancestor listener.
...
- **WinUI 3** (the reason this recipe exists): Build the scrim as a
  ...
  layer while the toolbar is a separate opaque sibling layer. Stack a
  `TextBlock` (style `SubtitleTextBlockStyle`, matching the `.heading` text
  role) above a `Border` toolbar (`CornerRadius="8"`, `BorderThickness="1"`,
  ...
  Up, Down). Since a WinUI `Border`/`Grid`'s `Tapped`/`PointerPressed`
  events are already marked handled for elements beneath it unless
  `e.Handled` is explicitly reset, getting `scrim-click-still-reaches-pane-selection`
  requires the scrim's own `Tapped` handler to explicitly re-invoke the
  pane's "select" command — there is no automatic bubble-to-ancestor the way
  AppKit's unhandled-`mouseDown` forwarding gives this view for free — call
  this out to implementors as a deliberate platform difference. Bind
  `IsEnabled` on the Add/Remove `Button`s to `canAdd`/`canRemove` the same
  way `refreshAvailability()` re-reads them, and leave the Done `Button`
  with no `AccessKey` bound, mirroring `done-button-has-no-key-equivalent`.

## Design Decisions

- Decision: Implement the scrim as a real, opaque `NSView` subview (its own
  layer background) rather than as an alpha applied to the pane's existing
  content.
  Rationale: Per the type's doc comment, a scrim that were only an alpha on
  the content would still let the content receive clicks and keystrokes,
  which "would be a pane you can rearrange and type into at the same time";
  a real covering view intercepts them, while still remaining a subview of
  the pane's backdrop so an unhandled click can travel up the responder
  chain to select the pane.
  Approved: pending
- Decision: Dim toward `SemanticPalette.nsColor(.windowBackground)` at 72%
  opacity rather than toward black.
  Rationale: Per the inline comment on `applyPalette(_:)`, dimming toward
  the window background "keeps a light theme light" — the content should
  read as behind something, not as switched off, which dimming toward black
  would suggest regardless of the active theme.
  Approved: pending
- Decision: Give the Done button no key equivalent, even though the type's
  doc comment notes Return, Enter, and Escape already leave arrange mode.
  Rationale: Per the doneButton-construction comment, none of those
  existing shortcuts is visible, and a toolbar that shows every other
  action arrange mode supports "owes the way out the same billing"; no
  single button can claim the default `\r` equivalent window-wide, since
  every pane in the window carries one of these buttons and a window with
  four default buttons would have none.
  Approved: pending
- Decision: Reuse `ComposableTabsMoveMenu` for the Move pull-down's items
  instead of building the four-direction list, labels, icons, and
  enablement rule inline in this view.
  Rationale: Per the class's own doc comment, the pane's gear menu needs the
  identical four directions, labels, arrows, and legality rule; keeping that
  knowledge in one shared object avoids a second place to miss when a
  direction is added or a name changes.
  Approved: pending
...
Live-region-announcements is flagged because `refreshAvailability()` changes
control enablement with no accessibility notification posted (see
**Announce state changes** under Accessibility). Differentiate-without-color
passes because disabled state is conveyed by AppKit's standard dimmed,
non-interactive rendering and Move items carry both text and an icon, not
color alone. Touch-target-size is failed because AppKit's default
`.rounded` bezel height is well under the 44×44pt/48×48dp threshold —
expected for this pointer/keyboard-driven macOS toolbar, not a defect on
this platform, but the threshold does apply to the touch-platform
translations in Platform Notes. Contrast-ratio is partial because every
color comes from `SemanticPalette`, whose actual resolved values are not
stated in this source. String-externalization is failed because "Add",
"Remove", "Move", and "Done" are hardcoded English literals with no
localization key. Main-actor-confined passes because the class is declared
`@MainActor`.

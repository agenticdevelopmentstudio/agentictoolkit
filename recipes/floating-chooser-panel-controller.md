...
- **falls-back-to-centering-without-visible-frame**: If the resolved screen
  has no `visibleFrame` available, the panel MUST be positioned with
  `NSWindow.center()` instead of the pointer-relative calculation.
...
- **resets-dismissing-flag-after-close**: `close()` MUST clear `isDismissing`
  back to `false` once its call to `super.close()` returns (or is skipped),
  so the next dismissal of a re-shown panel MUST be treated as a fresh
  entry.
...
| Dismissing | `isDismissing == true`, from entry into `close()` until its call to `super.close()` returns; a nested `close()` call or a `windowDidResignKey` notification arriving during this window MUST have no effect. |
...
| floating-chooser-panel-controller-002 | pins-content-to-container-edges | Inspect the container's view constraints after `init` | Exactly four constraints pin the content view's top/leading/trailing/bottom to the container view, each with constant 0. |
...
| floating-chooser-panel-controller-016 | activates-app-before-showing | Call `show()` with another app frontmost | `NSApp.activateUnlessQuiet()` runs before `showWindow(nil)`/`makeKeyAndOrderFront(nil)`. |
| floating-chooser-panel-controller-017 | respects-quiet-activation | `QuietWindowPresentation.isEnabled == true`; call `show()` | `activate(ignoringOtherApps:)` is not invoked; the window still shows and orders front. |
...
| floating-chooser-panel-controller-020 | defaults-initial-focus-hook-to-no-op | Call `show()` on the base class directly (no override) | No first-responder change beyond AppKit's default; no crash or side effect. |
...
| floating-chooser-panel-controller-024 | ignores-focus-loss-while-already-dismissing | `isDismissing == true`; window resigns key during that window | `close()` is not invoked a second time. |
...
| floating-chooser-panel-controller-029 | defaults-panel-will-close-hook-to-no-op | Close the base class directly (no override) | No crash or side effect beyond the standard close. |
...
- **Null/empty input**: `show()` guards `window` being `nil` and returns
  without effect if so (MUST) — in practice the window always exists once
  `init` has run, but the guard is present. If no screen contains the mouse
  location and `NSScreen.main` is also unavailable, `position(_:)` falls
  back to `window.center()` rather than computing an invalid origin (MUST).
...
Decision: Guard reentrancy with an owned `isDismissing` boolean rather than
inferring "already closing" from window state.
...
Decision: Keep `.closable` in the style mask (which puts a close button in
the frame) and then hide the close/miniaturize/zoom buttons explicitly,
...
Rationale: `close()` runs on every dismissal, including a plain focus loss;
a chooser is expected to reopen as the same instance rather than being
rebuilt from scratch each time, so the window and its controller must
...
Rationale: The chooser can be invoked with no window of this app on screen
at all (a system-global shortcut, or an extension request), so there may be
no key window's screen to anchor to; the pointer is the one reliable proxy
for the screen the user is currently looking at.
Approved: pending

...
| [input-sanitization](agenticdevelopercookbook://compliance/security#input-sanitization) | partial | The four hardcoded provider base URLs are fixed HTTPS literals, but `customBaseURL`/`baseURL`/`openAIBaseURL` accepted from a caller for `.custom` and OpenAI-compatible/Ollama routes carry no scheme validation before an API key is attached to a request against them (see the `custom-endpoint-scheme-validation` open question). |

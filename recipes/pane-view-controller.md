...
- **host-change-refreshes-controls**: Setting `host` to a new value MUST re-run `refreshControlAvailability()`.
...
- **title-change-callback-installed**: The pane MUST install its own closure into the content's `onPaneTitleChange` (when implemented), during `wireContentCallbacks()`, so the title bar updates without polling.
...
- **spacing-defaults-to-inherited**: A pane with no stored spacing override MUST apply `inheritedPaneSpacing`, which itself defaults to an all-zero `Spacing` unless the content implements `PaneContentSpacingConsuming`.
...
- **title-bar-trailing-active-except-rail**: The title bar's trailing constraint to the container MUST be active for every pane shape except a pane minimized to a horizontal (leading/trailing) edge, where it MUST be inactive.
...
- **Font**: Not set directly in this file; the title bar's own label typography belongs to `PaneTitleBarView` (`related`), out of this recipe's scope.
...
| pvc-016 | clamped-pane-yields-width | Set `clampsToContainer = true` before loading the view | The title bar's, and its close button's, horizontal content-compression-resistance priority is `1` |
...
| pvc-019 | title-follows-content | Content implementing `PaneTitleProviding` changes `paneTitle` from "Files" to "Sources" after load | `titleBar.title` updates to "Sources" without any poll from the pane |
| pvc-020 | title-change-callback-installed | Same setup as pvc-019 | The update happens strictly in response to `onPaneTitleChange` firing, not on a timer or the next layout pass |
...
| pvc-024 | gear-installed-trailing | Load any pane | `titleBar.gearView` is non-nil |
...
| pvc-031 | spacing-defaults-to-inherited | Content not implementing `PaneContentSpacingConsuming`, no stored override | The resolved spacing equals `inheritedPaneSpacing` (an all-zero `Spacing` when the content also has no override of its own) |
...
| pvc-033 | spacing-non-consumer-gets-pane-applied-gap | Content not implementing `PaneContentSpacingConsuming`; a stored override of `top: 10` | `contentEdgeConstraints.top.constant == 10` |
...
| pvc-037 | apply-resolved-spacing-is-idempotent | Call `applyResolvedSpacing()` twice in a row with nothing changed in between | The spacing control's displayed value and the content's edge-constraint constants are identical after both calls |
...
| pvc-045 | appearance-methods-noop-before-load | Same setup as pvc-044, checked immediately after `setMinimized` and before loading | `isViewLoaded == false` |
| pvc-046 | title-bar-trailing-active-except-rail | Compare `titleBarTrailing.isActive` after minimizing to `.leading` versus to `.top` | `false` for `.leading`; `true` for `.top` |
...
- **Null/empty input** (MUST): `makeContentViewController()` returning `nil` results in a working, empty pane rather than a failure (`no-content-is-supported`). A spacing-override JSON string that fails to decode, or a minimize-edge string that fails to parse as a `PaneEdge`, is treated as absent rather than as an error.
- **Boundary values** (MUST): A stored spacing override outside the `0...40` range — written by an older build, or edited by hand — is clamped back into range on read, never rejected outright. `contentInset` of `0`, the default, means `minimizedThickness(for:)` reduces to exactly the docked chrome's own thickness (26pt for a vertical edge, 28pt for a horizontal one) with no border allowance added.

...
domain: agentictoolkit://recipes/status-web-src-lib-overall
type: ingredient
version: 1.0.0
status: review
language: en
...
depends-on: []
related:
- agentictoolkit://recipes/status-server-monitor-overall
...
Within status-web, `OverallStatus` is re-exported by `src/types.ts` and types the `overall` field of `StatusResponse`, the shape of the payload the status server returns. No status-web module calls `computeOverall` at runtime; the verdict a page shows arrives already computed in `StatusResponse.overall`. The status server owns the authoritative rollup in its own `monitor/overall.ts` (see [Status Server Monitor Overall](agentictoolkit://recipes/status-server-monitor-overall)), which carries the same `computeOverall` plus an extra `publicOverall` step that this client copy does not have.

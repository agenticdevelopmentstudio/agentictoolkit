...
domain: agentictoolkit://recipes/git-client-projects-project-checkout
type: ingredient
version: 1.0.0
status: review
language: en
...
(`ProjectCheckout.swift` lines 4-6). It is a small `Hashable`, `Sendable`
...
- **checkout-identity-fields**: `ProjectCheckout` MUST expose exactly three
  ...
  `isMain: Bool` — and no others (`ProjectCheckout.swift` lines 7-10).
...
- **resolution-not-standardization**: `init(directory:branch:isMain:)` MUST
  ...
  (`ProjectCheckout.swift` lines 12-19, 21).
- **hashable-includes-all-fields**: Two `ProjectCheckout` values MUST
  ...
  stored properties (`ProjectCheckout.swift` line 7-10).
- **sendable-value-type**: `ProjectCheckout` MUST be safe to read, compare,
  ...
  mutable state (`ProjectCheckout.swift` line 7-10).
...
- **identifier-ignores-branch-and-main-flag**: `identifier` MUST depend only
  ...
  (`ProjectCheckout.swift` lines 26-27, 28-34).
- **display-name-prefers-branch**: `displayName` MUST return `branch` when
  ...
  is `nil` (`ProjectCheckout.swift` lines 36-38).
- **bare-worktrees-excluded**: `checkouts(from:)` MUST exclude every
  ...
  (`ProjectCheckout.swift` lines 40-44).
- **checkouts-preserve-order**: `checkouts(from:)` MUST return the
  ...
  the `worktrees` argument (`ProjectCheckout.swift` lines 42-44).
- **checkouts-projection-is-lossy-by-design**: `checkouts(from:)` MUST build
  ...
  (`ProjectCheckout.swift` lines 41-45).
...
| git-client-projects-project-checkout-006 | checkouts-projection-is-lossy-by-design | Same input as vector 005: `linked` carries `head: "def"` and `isDetached: false` on the source `GitWorktree`. | The `ProjectCheckout` produced for `linked` has no `head` or `isDetached` property to inspect at all — `ProjectCheckout` declares no such stored property (`ProjectCheckout.swift` lines 7-10, 41-45). |
| git-client-projects-project-checkout-007 | hashable-includes-all-fields, identifier-ignores-branch-and-main-flag | Not exercised by an existing test; derived directly from the type declaration: `a = ProjectCheckout(directory: "/repo/a", branch: "main", isMain: true)`, `b = ProjectCheckout(directory: "/repo/a", branch: "other", isMain: true)` (`ProjectCheckout.swift` line 7-10, no custom `Equatable`/`Hashable`). | `a == b` is `false` and `a.hashValue != b.hashValue` in the general case, while `a.identifier == b.identifier` is `true` — the same directory yields one `identifier` but two distinct, unequal `ProjectCheckout` values because `branch` differs. |
...
- **Null and empty input**: `branch == nil` MUST fall back `displayName` to
  ...
  string (`ProjectCheckout.swift` lines 36-38). MUST. An empty `worktrees`
  ...
  (`ProjectCheckout.swift` lines 41-45). MUST.
- **Boundary values**: A `directory` whose `path` is the single-character
  ...
  (`ProjectCheckout.swift` lines 29-33). MUST.
- **Concurrent access**: `ProjectCheckout` MUST be readable, comparable, and
  ...
  shared mutable state (`ProjectCheckout.swift` line 7-10, 26-38). MUST.
  ...
  with one another (`ProjectCheckout.swift` lines 41-45). MUST.
- **Error states**: `resolvingSymlinksInPath()` is a non-throwing API, and
  ...
  than a validated one (`ProjectCheckout.swift` lines 20-24). MUST (this is
...
- **Duplicate worktree entries**: When `worktrees` passed to
  ...
  projection with no `Set` or dedup step (`ProjectCheckout.swift` lines
  41-45). MUST.
...
- **Data collected**: A `ProjectCheckout` value holds the caller's local
  ...
  `branch` name (`ProjectCheckout.swift` lines 8-9). Both are local
...
(`ProjectCheckout.swift` lines 12-19).
**Approved**: pending
...
(`ProjectCheckout.swift` lines 26-27).
**Approved**: pending
...
the full worktree record (`ProjectCheckout.swift` lines 40-45).
**Approved**: pending

...
summary: A read-only detail pane that shows a placeholder when nothing is selected,
  and a selected file or directory's icon, name, path, size, modification date, and
  type when one is.
platforms:
- swift
- macos
...
- **grid-label-color**: Every metadata grid row's label (e.g. "Path:", "Size:") MUST render in the theme's `secondaryText` color, right-aligned within the label column.
...
- **Font**: Placeholder message — theme `heading` role (default style: 15pt, semibold). Header name — theme `title` role (default style: 22pt, semibold). Header type description and every grid label/value — theme `caption` role (default 11pt regular) for the header type description, theme `body` role (default 13pt regular) for the grid. The placeholder icon is a literal 48pt system image font, and the header icon a literal 40pt system image font — both set via `Image(...).font(.system(size:))` directly rather than through `theme.font(_:)`, so, unlike every text element in this view, neither icon's size scales with the theme's `sizeScale` text-size preference.
...
| content-viewer-view-024 | type-description-package | `isPackage = true`, extension `catnip-proj`, `config.packageDisplayNames = ["catnip-proj": "Catnip IDE Project Package"]` | Type description reads "Catnip IDE Project Package" |
...
- **Null input — no children**: `node.children == nil` (every file, every package, and any directory whose children were read to empty) omits the "Items:" row entirely. MUST (see `grid-items-row-conditional`).
...
- **Boundary — very long path**: `node.url.path` longer than fits 3 lines is clipped to 3 lines with default tail truncation, but because `textSelection(.enabled)` is set on that value, the full untruncated string remains selectable and copyable regardless of what is visually clipped. MUST.
...
- **Decision**: The header icon's color maps file-extension "brand" associations onto the theme's status roles: `swift`/`json` land on `warning`, `md`/`markdown` on `accent`.
  **Rationale**: The theme's `SemanticPalette` has no per-language brand-color role, so `headerIconColor` reuses the nearest status role the same way `SwiftUIPalette.color(named:)` maps conventional color names onto roles elsewhere in the toolkit (orange/yellow → `warning`, blue → `accent`); `swift` and `json` landing on the same tone is a direct consequence of that shared mapping, not a special case written for this view.
  **Approved**: pending

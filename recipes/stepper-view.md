...
- **requires-designated-initializer**: Component MUST NOT support construction
...
- **Label requirements**: NEEDS REVIEW: `stepper` has no accessibility
...
|----|-------------|-------|----------|
| stepper-view-001 | arranges-row-layout | Construct `StepperView` with any `viewModel` | `label`, `valueLabel`, and `stepper` are all subviews of a single row view that is pinned to the component's edges, in that left-to-right order; no other layout container appears |
...
| stepper-view-003 | builds-value-label-monospaced | Construct with any `viewModel` | `valueLabel` was built via `ComposableSettings.makeValueLabel(monospacedDigits: true)` (its text role resolves to the code/monospaced style) |
...
| stepper-view-016 | requires-designated-initializer | Attempt `StepperView(coder: someCoder)` | The call traps with a fatal error; no instance is returned |
| stepper-view-017 | rejects-frame-only-initialization | Attempt `StepperView(frame: .zero)` | The call traps with a fatal error; no instance is returned |
...
- **Null/empty input**: `viewModel` (`RangeViewModel<Int>`) is a non-optional,
  ...
  `viewModel.title` produces a `label` with an empty string and no crash.
  MUST: the component provides, and needs, no nil-handling path for its one
...
- **Contradictory bounds (`minValue > maxValue`)**: `StepperView.swift`
  ...
  `stepper.minValue`/`stepper.maxValue` with no validation or reordering; the
  component MUST NOT be assumed to guard against an inverted range — the
  resulting behavior in that case is `NSStepper`'s own unspecified handling
  of an inverted bound, not something this file coerces or clamps (contrast
  `IntegerFieldView`, which explicitly detects and skips clamping on this
  case).
...
- **Overwritten external observer**: `viewModel.onChange` is a single closure
  property. `StepperView`'s initializer unconditionally assigns
  ...
  the same closure-overwrite behavior `CheckboxView` and `IntegerFieldView`
...
| Differentiate Without Color | Not applicable: the component conveys no state through color alone — the current value is shown as digits in `valueLabel`, and reaching a bound is communicated by the stepper's own arrow-disable rendering, not a color-only signal introduced by this component. |
...
- Decision: Set `valueLabel`'s horizontal content compression resistance to
  `.required` while leaving `label` and `stepper` at their default
  priorities.
  Rationale: In a tight row, `NSStackView` shrinks the lowest-resistance view
  first; a `.required` floor on `valueLabel` keeps the numeric value — the
  one piece of information the row cannot afford to truncate — from being
...
- Decision: Disable wraparound (`stepper.valueWraps = false`).
  ...
  Approved: pending
- Decision: `init(frame:)`'s fatal error is left with the source's own
  literal message text, `"init(frame frameRect: NSRect"` (missing its
  closing parenthesis), rather than a corrected string.
  Rationale: The recipe documents the code as it is. The message is a
  copy-paste artifact of the initializer's own signature, unlike the
  properly formed `"init(coder:) has not been implemented"` message on
  `init?(coder:)` in the same file; it has no behavioral effect beyond the
  text shown if the fatal error ever fires, and implementers on other
  platforms should write their own clear message rather than port this text.
  Approved: pending

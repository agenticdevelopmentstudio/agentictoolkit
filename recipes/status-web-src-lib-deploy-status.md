...
domain: agentictoolkit://recipes/status-web-src-lib-deploy-status
type: ingredient
version: 1.0.0
status: review
language: en
...
The module's own comment calls it "A HAND MIRROR of the server's deploy-status.ts (there is no shared build between the two packages)". The server original is specified in [Status Server Monitor Deploy Status](agentictoolkit://recipes/status-server-monitor-deploy-status). `deploy-status-parity.test.ts` imports both modules and asserts they agree across the full phase space.

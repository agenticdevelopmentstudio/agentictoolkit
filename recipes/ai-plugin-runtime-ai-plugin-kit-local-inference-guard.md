...
|----|-------------|-------|----------|
| local-inference-guard-001 | guard-refusal-cases, guard-refusal-description | `AIGuardError.blocked("x").errorDescription` and `AIGuardError.deferred("y").errorDescription` (`LocalInferenceGuard.swift`, lines 910-920). | Both return the associated reason unchanged: `"x"` and `"y"` respectively. |
| local-inference-guard-002 | guard-refusal-non-transport | Inspect `DaemonAIChat.complete`/`completeViaPlugin` (`DaemonAIChat.swift`, lines 122-125 and 247-250). | Both throw sites read `throw AIGuardError.blocked(reason)` / `.deferred(reason)` directly — never wrapped in `ChatError.providerError` — so `completeViaCLI` (the `claude -p` fallback) is never reached from either throw. |
| local-inference-guard-003 | shared-default-instance, injectable-dependencies | `LocalInferenceGuard.shared` vs. `LocalInferenceGuardTests.makeGuard(tags:pressure:)`, which builds `LocalInferenceGuard(catalog: LocalModelCatalog(fetcher: { ... }), memory: FixedMemory(...))` (`LocalInferenceGuardTests.swift`, lines 17-25). | The injected initializer accepts substitute `catalog`/`memory` values and every test in the file runs with no live `SystemMemoryMonitor.shared` or network access. |
| local-inference-guard-004 | threshold-settings-override, threshold-fallback-on-missing-or-invalid, disk-size-lookup-delegation, verdict-computation-delegation | `blocksOverBudgetModel`: model `"big:latest"` (51 GB on disk), RAM 64 GB, `settings: { _ in nil }`, pressure `.normal` (`LocalInferenceGuardTests.swift`, lines 27-36). | `.block` verdict — est. resident 61.2 GB ≈ 95.6% of RAM, at or above the default 50% block threshold. |
| local-inference-guard-005 | threshold-settings-override | `thresholdOverridesComeFromSettings`: same model/RAM, `settings: { key in key == ModelFitPolicy.blockPctKey ? "99" : nil }` (lines 38-46). | `.allow` — 95.6% is below the overridden 99% block threshold. |
| local-inference-guard-006 | threshold-fallback-on-missing-or-invalid | `verdict(model: "big:latest", baseURL: ..., settings: { key in key == ModelFitPolicy.blockPctKey ? "not-a-number" : nil })` with the same 51 GB / 64 GB setup as vector 004 (`LocalInferenceGuard.swift`, lines 959-962). | `Int.init("not-a-number")` fails, `flatMap` yields `nil`, so the default 50% block threshold applies — outcome `.block`, matching vector 004. |
| local-inference-guard-007 | unknown-size-fail-open, disk-size-lookup-delegation | `unknownSizeFailsOpen`: `tags: nil` (fetcher throws), pressure `.normal` (lines 59-64). | `.allow` — `nil` diskBytes yields a `nil` tier, and the pressure component alone allows. |
| local-inference-guard-008 | verdict-computation-delegation | `defersUnderPressureEvenWithUnknownSize`: `tags: nil`, pressure `.critical` (lines 48-57). | `.deferred`, even though the size is unknown — pressure is checked ahead of the size tier. |
| local-inference-guard-009 | pressure-only-recheck | Call `guardActor.pressureVerdict()` directly with the memory monitor's `pressureLevel` set to `.warning`, then to `.normal` (`LocalInferenceGuard.swift`, lines 981-986). | Returns `.deferred(reason: "memory pressure is warning; deferring local inference")` for `.warning`, and `.allow` for `.normal` — independent of any model or disk-size input. |
| local-inference-guard-010 | warn-tier-log-emission, warn-tier-log-scope | Call `verdict` with a disk size landing in the warn band under default thresholds (e.g. 20 GB on 64 GB RAM ⇒ est. 24 GB ≈ 37.5% of RAM) (lines 964-974). | Exactly one `Logger.notice` call fires, naming the model and its estimated footprint; a repeat call with a size in the `.ok` or `.block` band, or with `nil` size, emits no such log line. |
...
| local-inference-guard-012 | mutual-exclusion, actor-isolated-mutable-state | `runExclusiveSerializes`: 4 concurrent `runExclusive` calls, each sleeping 20ms inside an `OverlapTracker` (lines 67-83). | `tracker.maxConcurrent == 1`. |
| local-inference-guard-013 | fifo-admission-order, direct-handoff-without-busy-gap | `waitersRunInFIFOOrder`: one holder plus three waiters parked in order, released in sequence (lines 85-113). | `order.values == [0, 1, 2]`. |
| local-inference-guard-014 | uncontended-fast-path | Call `runExclusive` on an idle guard with no operation in flight. | The operation runs immediately with `waiterCount == 0` throughout — no continuation is ever created (lines 1011-1015). |
| local-inference-guard-015 | post-acquire-cancellation-check, release-on-every-exit-path | Call `runExclusive` from a `Task` cancelled before its body starts running, on an otherwise idle guard. | `Task.checkCancellation()` (line 1005) throws `CancellationError` immediately after the uncontended `acquire()` succeeds; `operation` never runs; `release()` still runs via `defer`, leaving `busy == false`. |
| local-inference-guard-016 | parked-cancellation-throws | `cancelledParkedWaiterThrowsWithoutRunning` (lines 115-146). | The cancelled waiter's `result` is `.failure(CancellationError)`, `ran.isRaised == false`, and `waiterCount == 0` immediately — without waiting for the holder to finish. |
| local-inference-guard-017 | handoff-race-cancellation-noop | Cancel a waiter's `Task` at the instant `release()` has already called `waiters.removeFirst().continuation.resume(returning: true)` for it. | `cancelWaiter` finds no matching `id` in `waiters` (line 1031) and is a no-op; the waiter proceeds as the new owner. |
| local-inference-guard-018 | default-deadline-value | Call `runExclusive` with no `deadline` argument. | `LocalInferenceGuard.defaultDeadline` (600 seconds) is the value threaded into `withDeadline` (lines 999-1002). |
| local-inference-guard-019 | deadline-enforced-per-operation, deadline-cancellation-preserves-handoff | `deadlineCutsOffHungOperation`: `deadline: 0.1`, a hung operation sleeping 60s, a second waiter queued behind it (lines 149-169). | The waiter's `runExclusive` resolves to `"ran"` well before 60s; the hung task's `result` is `.failure(AIGuardError.deferred)`. |
| local-inference-guard-020 | deadline-clamped-to-nonnegative | `runExclusive(deadline: -5.0) { "ok" }`. | `max(0, deadline)` (line 1053) yields a zero-duration sleep rather than a negative-duration crash — the timer branch fires essentially immediately. |
| local-inference-guard-021 | operation-result-propagation | `runExclusive { throw MyError() }` and, separately, `runExclusive { 42 }`, on an idle guard with a large deadline (lines 1060-1063). | The thrown `MyError` propagates unchanged in the first case; `42` is returned unchanged in the second. |
| local-inference-guard-022 | sendable-operation-and-result | Attempt `runExclusive { NSMutableString() }` (a non-`Sendable` return type) in a strict-concurrency build (line 999). | Fails to compile — the `T: Sendable` generic constraint rejects a non-`Sendable` result type. |
| local-inference-guard-023 | waiter-count-internal-visibility | From `LocalInferenceGuardTests.swift` (`@testable import AIPluginKit`), read `guardActor.waiterCount` (used throughout, e.g. lines 105, 131, 142). | Compiles and reads the live count; a module that only does `import AIPluginKit` (no `@testable`) fails to compile the same access, since `waiterCount` (line 944) carries no `public` modifier. |

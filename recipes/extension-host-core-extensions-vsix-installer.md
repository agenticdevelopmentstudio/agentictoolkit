...
title: VSIXInstaller
domain: agentictoolkit://recipes/extension-host-core-extensions-vsix-installer
type: ingredient
version: 1.0.0
status: review
language: en
...
- agentictoolkit://recipes/extension-host-core-extensions-vsix-archive
- agentictoolkit://recipes/extension-host-core-extensions-open-vsx-catalog
...
version winning after an update that only added a folder) (source lines
9–26).
...
nor a user's file selection can be trusted to agree with (source lines 78–174,
176–311).
...
- **default-host-version**: `init`'s `hostVersion` parameter MUST default to
  ...
  1, minor: 138, patch: 0)` (lines 36–39, 66–68; `ExtensionRegistry.swift`
  lines 52–53).
...
- **sendable-value-type**: `VSIXInstaller` MUST be declared as a `Sendable`
  ...
  `SemanticVersion`, and an immutable `@Sendable` closure (line 27).
- **registry-install-order**: `install(_:using:)` MUST decide installability
  ...
  artifacts the record named (lines 78–137).
- **registry-installability-gate**: `install(_:using:)` MUST throw
  ...
  without making any request (lines 91–94).
- **registry-requires-universal-build**: `install(_:using:)` MUST throw
  ...
  `detail.universalDownloadURL` is `nil`, without making any request (lines
  95–97).
- **signature-pair-completeness**: `install(_:using:)` MUST throw
  ...
  key with no signature (lines 99–120).
- **artifact-fetched-only-if-named**: `install(_:using:)` MUST fetch
  ...
  the URL that failed (lines 122–137).
- **blocking-verification-hop**: `install(_:using:)` MUST run archive
  ...
  (lines 139–174; `BlockingWork.swift` lines 8–34).
- **local-install-entry-point**: `install(archive:verification:
  ...
  `nil` for the latter case (lines 176–189).
- **scratch-directory-lifecycle**: `install(archive:...)` MUST create a
  ...
  successful or not (lines 191–194).
...
- **payload-directory-required**: `install(archive:...)` MUST throw
  ...
  expanded)` does not exist — a zip that is not a `.vsix` (lines 201–204).
- **manifest-read-and-localized**: `readManifest(in:)` MUST read
  ...
  with a plain `JSONDecoder` (lines 351–369).
...
- **manifest-malformed**: `readManifest(in:)` MUST throw
  ...
  decode as `ExtensionManifest` (lines 356–368).
...
- **identity-fields-validated**: `install(archive:...)` MUST call
  ...
  nothing else on any path (lines 226–232, 343–347;
  `ExtensionIdentityComponent.swift` lines 38–44).
- **directory-naming-convention**: `VSIXInstaller.directoryName(identifier:
  ...
  this function (lines 234–235, 313–330).
- **destination-containment-double-checked**: `install(archive:...)` MUST
  ...
  and be caught here (lines 239–259).
- **install-directory-created**: `install(archive:...)` MUST create
  ...
  treated as an edge case (line 261; test `createsTheInstallDirectory`).
- **recovery-runs-before-scan**: `install(archive:...)` MUST call
  ...
  visible to the supersede sweep rather than invisible and unreclaimed (lines
  263–269).
- **reinstall-idempotent**: `install(archive:...)` MUST succeed, not throw a
  ...
  bytes have already passed verification (lines 271–277; test
...
- **supersede-by-manifest-identity**: `otherInstallDirectories(claiming:
  ...
  it is still recognized as superseded (lines 371–401; test
...
- **supersede-non-fatal-removal**: `install(archive:...)` MUST still return a
  ...
  would be false (lines 283–300; test
...
- **move-into-place-rollback**: `moveIntoPlace(_:to:)` MUST attempt to move
  ...
  aside's last path component (lines 423–457).
...
- **aside-naming**: the aside a replaced directory is moved to MUST be named
  ...
  exists for the moment between the two renames (lines 411–417, 461–473).
- **recovery-sweep-scope**: `recoverInterruptedInstalls()` MUST list
  ...
  destination (when it does not) (lines 514–552).
- **recovery-non-aside-entries-untouched**: `Self.replacedName(ofAside:)` MUST
  ...
  touched by a recovery sweep (lines 476–487; test
...
- **recovery-failure-logged**: `recoverInterruptedInstalls()` MUST log, at
  ...
  only surviving copy of the extension (lines 539–549).
- **recovery-idempotent**: `recoverInterruptedInstalls()` MUST be safe to call
  ...
  actor-free call path (lines 510–513).
- **installation-record-shape**: On success, `install(archive:...)` MUST
  ...
  call's own inputs (lines 302–311, 560–612).
- **runnable-here-computed**: `VSIXInstallation.runnableHere` MUST equal
  ...
  install, since the extension's declarative contributions still work (lines
  310, 574–584; test `runnableHereFollowsTheEntryPoint`).
- **install-source-shape**: `VSIXInstallation.Source` MUST be either
  ...
  + "/" + detail.name` and `detail.version` (lines 156–157, 586–591).
- **directory-name-not-independently-safe**: `directoryName(identifier:
  ...
  above, in the caller (lines 322–330).
...
- **logging-conformance**: `VSIXInstaller` MUST conform to `Loggable`,
  ...
  bundleIdentifier` resolves to (`"nil"` when there is none) (lines 555–557;
...
- **Malformed input (hostile manifest fields)**: `identifier` or `version`
  ...
  (lines 226–259; tests `hostileVersionIsRefused`, `hostileNameIsRefused`,
...
- **Error states (dependency unavailable)**: On the registry path, a
  ...
  before any verification or file-system step in this component begins (lines
  122–137). This component defines no retry, timeout, or cancellation of its
...
developer-facing diagnostics, not strings a person using the app would read
(lines 294–298, 442–447, 544–548, 614–650). Separately, `readManifest(in:)`
...
any string this component itself displays (lines 356–365).
...
- **Storage**: The extension's expanded payload is written, unencrypted, as
  ...
  shipping app (doc comment lines 29–30). A superseded copy and, briefly, a
...
- **Transmission**: `VSIXInstaller` itself makes no network request; on the
  ...
  Privacy section states about that transmission applies here as well (lines
  122–137).
...
| Event | Level | Message |
|-------|-------|---------|
| A superseded directory was left on disk because it could not be removed after an otherwise successful install | warning | `Installed <identifier> but could not remove the superseded copy at <path>: <reason>` (lines 294–298) |
| `moveIntoPlace`'s rollback (moving the aside back to `destination`) failed after the payload move into `destination` had also failed | error | `Could not put the previous <destination.lastPathComponent> back from <aside.lastPathComponent>: <reason>` (lines 442–447) |
| `recoverInterruptedInstalls()` failed to restore or discard a crash-orphaned aside | error | `Could not finish the interrupted replacement of <replaced>: <reason>` (lines 544–548) |
...
unsafe to press (doc comment lines 48–52; `moveIntoPlace`, lines 403–457).
**Approved**: pending
...
`rename(2)` actually resolves (lines 239–246).
**Approved**: pending
...
`<publisher>.<name>-<version>` (`ExtensionResourcePath.swift`
lines 112–127; test `aPrivateRootedInstallDirectoryStillInstalls`).
**Approved**: pending
...
content (doc comment lines 371–377; test `supersedesAHandNamedDirectory`).
**Approved**: pending
...
can move it back by hand and nothing else on disk says where it went (lines
433–450).
**Approved**: pending
...
recovery that did the safe thing) as having failed (lines 283–300, 539–549).
**Approved**: pending

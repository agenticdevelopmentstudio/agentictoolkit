...
domain: agentictoolkit://recipes/status-web-src-telemetry
type: ingredient
version: 1.0.0
status: review
language: en
...
The [useTelemetry](agentictoolkit://recipes/status-web-hooks-use-telemetry) hook uses only `cache` and `source`. Swapping the backend means changing one line in `client.ts`, and the hook, cache and panels stay the same. The server side that answers `/telemetry`, `/errors` and `/analytics` is covered by [Status Server Telemetry](agentictoolkit://recipes/status-server-telemetry). The `StatusApiClient` that every source reads through is covered by [Status Web API](agentictoolkit://recipes/status-web-api).

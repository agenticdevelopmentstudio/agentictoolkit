...
domain: agentictoolkit://cookbook/hub/features/invitations
type: ingredient
version: 1.0.1
status: review
language: en
...
- agentictoolkit://cookbook/hub/features/customers
...
- **accept-is-authenticated**: `accept` calls `authedJson` to POST `{
  ...
  entirely `authedJson`'s (documented in `agentictoolkit://cookbook/hub/features/authentication`),
  not redefined here.
...
- **Offline/disconnected state**: None of the seven given sources implement
  ...
  `agentictoolkit://cookbook/hub/features/authentication`), not a general
...
| Platform | Path |
|----------|------|
| apple | `packages/apple/AgenticToolkit/Hub/Features/Invitations/` |
| web | `packages/web/packages/data/src/invitations/` |

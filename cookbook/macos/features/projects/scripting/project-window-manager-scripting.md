...
domain: agentictoolkit://cookbook/macos/features/projects/scripting/project-window-manager-scripting
type: ingredient
version: 1.0.1
status: review
language: en
...
- macos
depends-on:
- agentictoolkit://cookbook/macos/scripting
...
- **window-object-specifier-key**: `ScriptableProjectWindow.objectSpecifier`
  ...
  "projectWindows") { self.uniqueID }` (`ScriptableProjectWindow.swift`; see `agentictoolkit://cookbook/macos/scripting` for that
...
- **tab-branch-empty-when-unresolved**: `ComposableTabsWindowController.scriptingTabs(branch:)`
  ...
  `agentictoolkit://cookbook/macos/features/projects/branch-controller`) all
...
- **Concurrent access**: Every operation in this file, and on all three
  ...
  **main-actor-confinement**); Cocoa Scripting itself dispatches command
  handlers on the main thread (see `agentictoolkit://cookbook/macos/scripting`),
...
- **Error states**: This file defines no `throws` function and surfaces no
  ...
  extension (see `agentictoolkit://cookbook/macos/features/projects/branch-controller`);
...
`GitClient.execute` (see `agentictoolkit://cookbook/macos/features/projects/branch-controller`),
...
- **WinUI 3**: There is no AppleScript/Cocoa Scripting equivalent on
  ...
  `agentictoolkit://cookbook/macos/features/projects/branch-controller`), folding
...
documented in `agentictoolkit://cookbook/macos/features/projects/branch-controller`.
**Approved**: pending

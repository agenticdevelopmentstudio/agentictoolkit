...
domain: agentictoolkit://cookbook/macos/features/projects/branch-controller
type: ingredient
version: 1.0.2
status: review
language: en
...
- **Concurrent access**: Every mutation of `currentBranch` and `panes` is
  ...
  `agentictoolkit://cookbook/macos/ui/view-controllers/file-browser/model/git`), which explicitly coalesces
...
- **Error states**: `GitClientError.executableNotFound`, `.launchFailed`,
  ...
  **refresh-always-reloads-panes**). The caller of the Refresh Status
  ...
  not this file — see `agentictoolkit://cookbook/macos/ui/view-controllers/file-browser/model/git`'s sibling
  `GitClient.swift`).
...
(see `agentictoolkit://cookbook/macos/ui/view-controllers/file-browser/model/git`'s sibling `GitClient.swift`),
...
- **WinUI 3**: Port `BranchController` as a plain C# class holding
  ...
  `agentictoolkit://cookbook/macos/ui/view-controllers/file-browser/model/git`). Replace `NSHashTable<TabPaneViewController>.weakObjects()`

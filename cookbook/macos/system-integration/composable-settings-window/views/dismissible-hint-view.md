...
domain: agentictoolkit://cookbook/macos/system-integration/composable-settings-window/views/dismissible-hint-view
type: ingredient
version: 1.1.2
status: review
language: en
...
(`agentictoolkit://cookbook/macos/system-integration/composable-settings-window/views/conditional-view`) conforms to for the same reason.
...
- Delayed dismissal (asynchronous setting round-trip): tapping
  ...
  `ConditionalView` recipe (`agentictoolkit://cookbook/macos/system-integration/composable-settings-window/views/conditional-view`)
...
the sibling `Badge` recipe (`agentictoolkit://cookbook/macos/ui/badge`) gives fully
...
- **Decision**: Copy the observed setting's current value directly into
  ...
  **Rationale**: A synchronous initial assignment prevents the view from
  ...
  recipe (`agentictoolkit://cookbook/macos/system-integration/composable-settings-window/views/conditional-view`) documents for its own

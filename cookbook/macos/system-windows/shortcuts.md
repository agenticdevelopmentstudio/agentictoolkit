...
domain: agentictoolkit://cookbook/macos/system-windows/shortcuts
type: ingredient
version: 1.0.1
status: review
language: en
...
depends-on: []
related:
- agentictoolkit://cookbook/system-windows-contexts
references:
...
Use it once per app that hosts window contexts: the host constructs one manager at feature start and retains it for the app's lifetime (`WindowContextsCoordinator.start()` does exactly this). The model's behavior behind each action belongs to [Window Matching System Windows Contexts](agentictoolkit://cookbook/system-windows-contexts); this recipe covers only the scheme and the dispatch.

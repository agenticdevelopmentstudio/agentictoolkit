...
domain: agentictoolkit://cookbook/data/gamification
type: ingredient
version: 1.0.1
status: review
language: en
...
**Rationale**: the source does not explain why. This is a notable divergence from the sibling component [Hub Domain: Ecosystem Config](agentictoolkit://cookbook/hub/features/ecosystem-config), whose `signin-apps.ts` and `feature-flags.ts` call `rethrowConflict` to turn a backend 409 into a friendly, entity-named message before it reaches a caller. This recipe records the observed divergence rather than inventing a motive the source does not state.
**Approved**: pending

...
domain: agentictoolkit://cookbook/status-server/config
type: ingredient
version: 1.0.1
status: review
language: en
...
| Platform | Path |
|----------|------|
| web | `packages/web/packages/status-server/src/config/` |
...
- **Decision**: this recipe's `related` field links to
  `agentictoolkit://cookbook/status-server/auth` one-directionally; the
  reverse link on `status-server-auth.md` was not added.
  **Rationale**: this recipe was authored under an explicit constraint to
  write only `cookbook/status-server/config.md` and touch no other file;

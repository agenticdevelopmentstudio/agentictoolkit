...
domain: agentictoolkit://cookbook/status-server/monitor/webhook-events
type: ingredient
version: 1.0.1
status: review
language: en
...
- agentictoolkit://cookbook/status-server/monitor/format
- agentictoolkit://cookbook/status-server/monitor/provider-deploy
- agentictoolkit://cookbook/status-server/monitor/live-buffer
- agentictoolkit://cookbook/status-server/monitor/fetch-vercel
- agentictoolkit://cookbook/status-server/monitor/fetch-railway
references:
...
- **vercel-created-at**: A returned Vercel row's `createdAt` field MUST be `toValidDate(event.createdAt) ?? new Date()` — the current wall-clock time (the webhook's receipt time) whenever `event.createdAt` is absent or does not parse to a finite `Date` via `toValidDate` (`./provider-deploy`, agentictoolkit://cookbook/status-server/monitor/provider-deploy). Per the source's own comment, "a webhook is often the only witness of a terminal state, so approximate time beats a dropped event (and an Invalid Date must never leave this boundary)."
...
| Platform | Path |
|----------|------|
| web | `packages/web/packages/status-server/src/monitor/webhook-events.ts` |

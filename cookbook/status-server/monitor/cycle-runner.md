...
domain: agentictoolkit://cookbook/status-server/monitor/cycle-runner
type: ingredient
version: 1.0.1
status: review
language: en
...
This file's only literal string is the hardcoded English `console.log` message it builds for a retention prune (maintenance-prune-logged-on-deletion). It routes through no localization mechanism, and unlike the Slack/Discord-rendered lines in `alerts.ts` (see the [Status Server Monitor Alerts](agentictoolkit://cookbook/status-server/monitor/alerts) recipe), this string's only audience is an operator reading container stdout, never an end user or a webhook receiver.
...
| Platform | Path |
|----------|------|
| web | `packages/web/packages/status-server/src/monitor/cycle-runner.ts` |

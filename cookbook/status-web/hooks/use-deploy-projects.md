...
domain: agentictoolkit://cookbook/status-web/hooks/use-deploy-projects
type: ingredient
version: 1.0.1
status: review
language: en
...
All requests go through the injected `StatusApiClient` (see [status-web-api](agentictoolkit://cookbook/status-web/api)), so paths are relative to the host's API base.

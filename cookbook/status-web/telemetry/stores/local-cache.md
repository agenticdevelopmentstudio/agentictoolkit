...
domain: agentictoolkit://cookbook/status-web/telemetry/stores/local-cache
type: ingredient
version: 1.0.1
status: review
language: en
...
The composition root `telemetry/client.ts` wires it as `export const cache: SnapshotCache = localCache;`, and the `useTelemetry` hook calls `cache.load()` once after mount and `cache.save(query.data)` whenever fresh data arrives (see [Status Web Hooks Use Telemetry](agentictoolkit://cookbook/status-web/hooks/use-telemetry)). Swapping the adapter is a one-line change at that root; nothing above the port moves.
...
| Platform | Path |
|----------|------|
| web | `packages/web/packages/status-web/src/telemetry/stores/local-cache.ts` |

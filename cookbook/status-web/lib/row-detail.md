...
title: Row Detail
domain: agentictoolkit://cookbook/status-web/lib/row-detail
type: ingredient
version: 1.0.1
status: review
language: en
...
depends-on: []
related:
- agentictoolkit://cookbook/status-web/lib/deploy-display
references: []
approved-by: ''
approved-date: ''
---
...
The module imports `hostOf` from `./url`, which returns `new URL(url).host` or the input unchanged when parsing throws. It also imports `platformLabel` from `./deploy-display`, specified in [Deploy Display](agentictoolkit://cookbook/status-web/lib/deploy-display). It has no state, no I/O and no side effects.

...
domain: agentictoolkit://cookbook/status-web/lib/self-check
type: ingredient
version: 1.0.1
status: review
language: en
...
depends-on: []
related:
- agentictoolkit://cookbook/status-web/hooks/use-integrations
...
- **Network failure, timeout, offline**: not applicable to this function; it does no I/O. Fetch failures belong to [useIntegrations](agentictoolkit://cookbook/status-web/hooks/use-integrations), which passes `undefined` or the last good report.
...
| Platform | Path |
|----------|------|
| web | `packages/web/packages/status-web/src/lib/self-check.ts` |

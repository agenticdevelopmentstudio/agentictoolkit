...
title: Deploy View
domain: agentictoolkit://cookbook/status-web/lib/deploy-view
type: ingredient
version: 1.0.1
status: review
language: en
...
The stale-in-flight test is `deployDtoUnconfirmed` from `./row-model`. The [Board Staleness](agentictoolkit://cookbook/status-web/lib/board-staleness) recipe shares its fail-closed clock rule.

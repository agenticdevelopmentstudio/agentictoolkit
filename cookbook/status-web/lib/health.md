...
domain: agentictoolkit://cookbook/status-web/lib/health
type: ingredient
version: 1.0.1
status: review
language: en
...
- agentictoolkit://cookbook/status-server/monitor/probe
references: []
approved-by: ''
approved-date: ''
---
...
The file is a byte-identical copy of `packages/web/packages/status-server/src/monitor/health.ts` ([status-server health](agentictoolkit://cookbook/status-server/monitor/health)). There the probe ([status-server probe](agentictoolkit://cookbook/status-server/monitor/probe)) calls `classify` with every endpoint result. Inside status-web only the `HealthStatus` type is imported: by `src/types.ts` (which re-exports it and uses it for endpoint `status` fields), `src/lib/uptime.ts` (`dayStatus`) and `src/lib/overall.ts` (`computeOverall`). No status-web runtime code calls `classify` or reads either constant. They are exercised only by `health.test.ts`. The module has no state, no I/O and no side effects.
...
| Platform | Path |
|----------|------|
| web | `packages/web/packages/status-web/src/lib/health.ts` |

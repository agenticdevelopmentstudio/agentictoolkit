...
title: Issue Sources
domain: agentictoolkit://cookbook/status-web/lib/issue-sources
type: ingredient
version: 1.0.1
status: review
language: en
...
- agentictoolkit://cookbook/status-web/lib/endpoint-kinds
references: []
approved-by: ''
approved-date: ''
---
...
Consumers in status-web: `hooks/use-source-filter.ts` seeds its selection from `ISSUE_SOURCES` ([useSourceFilter](agentictoolkit://cookbook/status-web/hooks/use-source-filter)); `lib/board-types.ts` types board rows with `IssueSource` ([Board Types](agentictoolkit://cookbook/status-web/lib/board-types)); `lib/row-model.ts`, `components/ActivityPanel.tsx`, `components/OverviewTab.tsx` and `components/GlobalPanel.tsx` read the labels, order and guard.

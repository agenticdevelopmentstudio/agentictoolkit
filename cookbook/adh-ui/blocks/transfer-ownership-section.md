...
domain: agentictoolkit://cookbook/adh-ui/blocks/transfer-ownership-section
type: ingredient
version: 1.1.2
status: review
language: en
...
related:
- agentictoolkit://cookbook/adh-ui/blocks/delete-entity-section
references: []
approved-by: ''
approved-date: ''
---
...
`noun` (`entityNoun.toLowerCase()`) is a casing transform, and casing is locale-sensitive — a
lowercase conversion can change meaning or produce the wrong result in some locales (e.g.
Turkish `I`/`ı`). This is a display transform of caller-supplied text, not an

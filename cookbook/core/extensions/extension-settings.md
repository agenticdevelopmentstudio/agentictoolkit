...
domain: agentictoolkit://cookbook/core/extensions/extension-settings
type: ingredient
version: 1.0.1
status: review
language: en
...
Not applicable: `disabledExtensionIdentifiers` stores extension identifier strings, not a credential or token. `isSecure` is not set at the call site and defaults to `false` (`ExtensionSettings.swift`; `UserSetting.swift`), so the value stays in local `UserDefaults` storage and is never routed to the Keychain, transmitted off-device, or otherwise treated as sensitive.
